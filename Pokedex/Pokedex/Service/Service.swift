...
class Service
{
    static let shared = Service() //shared instance of class
    let BASE_URL = "https://pokedex-bb36f.firebaseio.com/pokemon.json" //JSON file URL
            ...
            do
            {
                ...
                for (key, result) in resultArray.enumerated() {
                    if let dictionary = result as? [String: AnyObject] {
                        ...
                        //print(pokemon.name) //array of pokemons name
                    }
                   // completion(pokemonArray)
                }
                //print(resultArray) //prints JSON file(data) in console
            }
            catch let error
            {
